...
import com.grami.directed_patterns.Dijkstra.*;
import com.grami.directed_patterns.dataStructures.DFSCode;
import com.grami.directed_patterns.dataStructures.Edge;
...
import com.grami.directed_patterns.dataStructures.IntFrequency;
import com.grami.directed_patterns.dataStructures.gEdgeComparator;
...
public class Searcher<NodeType, EdgeType> 
{
	...
	{
		Graph graph = new Graph(1,freqThreshold);
		graph.loadFromFile_Ehab(path);
		setup(graph, freqThreshold, shortestDistance);
	}
	...
	private void setup(Graph graph, int freqThreshold, int shortestDistance)
	{
		...
		else
		{
			...
			singleGraph.printFreqNodes();
			singleGraph.setShortestPaths(d);
		}
	}
		...
		for (Iterator<  java.util.Map.Entry< Integer, HashMap<Integer,myNode> > >  it= freqNodesByLabel.entrySet().iterator(); it.hasNext();) 
		{
			...
			for (Iterator<myNode> iterator = tmp.values().iterator(); iterator.hasNext();) 
			{
				...
				for (Iterator<Integer>  iter= neighbours.keySet().iterator(); iter.hasNext();) 
				{
					...
					final GSpanEdge<NodeType, EdgeType> gedge = new GSpanEdge <NodeType, EdgeType>().set(0, 1, labelA, 0, labelB, 1, firstLabel, secondLabel);
