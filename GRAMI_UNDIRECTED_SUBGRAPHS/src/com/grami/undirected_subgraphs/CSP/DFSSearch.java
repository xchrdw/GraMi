...
package com.grami.undirected_subgraphs.CSP;
...
import com.grami.undirected_subgraphs.automorphism.Automorphism;

import com.grami.undirected_subgraphs.pruning.SPpruner;
...
import com.grami.undirected_subgraphs.dataStructures.HPListGraph;
import com.grami.undirected_subgraphs.dataStructures.Query;
import com.grami.undirected_subgraphs.dataStructures.myNode;
import com.grami.undirected_subgraphs.dataStructures.IntIterator;
...
public class DFSSearch
{
	...
	//for automorphisms and non-cached com.grami.undirected_subgraphs.search
	...
	public int hasBeenPrecomputed(Variable[] autos,int[] preComputed,int index) //if returns same index should com.grami.undirected_subgraphs.search in it!!
	{
	...
	public void searchExistances()
	{
		...
		for (int i = variables.length-1; i >=0 ; i--) 
		{
			...
			for (Iterator<myNode> iterator = firstList.values().iterator(); iterator.hasNext();)
			{
				...
				//if already marked dont com.grami.undirected_subgraphs.search it
			...
			if(result[index].getList().size()<minFreqThreshold)
			{
				...
				else //TRULY SEARCH INTO IT!!
				{
					...
					for (int j = 0; j < tmp.size(); j++) 
					{
						...
						if(Settings.isDecomposeOn==true) //decomposition is ON !!!
						{
							...
							for (int k = 0; k < maps.size(); k++) //iterate over edges removed!! 
							{
								...
								for (Iterator<Entry<HPListGraph<Integer, Double>, ArrayList<Integer>>> iterator = edgeRemoved.entrySet().iterator(); iterator.hasNext();) 
								{
									...
									myNode firstNode=tmp.get(j);//current node to com.grami.undirected_subgraphs.search for (previously timed out)
									...
									//com.grami.undirected_subgraphs.search the pruned domains for any existence of the instance
						...
						//if one of the the pattern subgraphs can not be satisfied, then no need to do com.grami.undirected_subgraphs.search as this node is already removed
						if(isExistant==false)
						{
							continue;
						}
						
						//continue the com.grami.undirected_subgraphs.search
