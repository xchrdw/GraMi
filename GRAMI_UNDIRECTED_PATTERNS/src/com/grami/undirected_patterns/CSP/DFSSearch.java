...
package com.grami.undirected_patterns.CSP;
...
import com.grami.undirected_patterns.automorphism.Automorphism;

import com.grami.undirected_patterns.pruning.SPpruner;

import com.grami.undirected_patterns.utilities.Settings;
import com.grami.undirected_patterns.utilities.Util;

import com.grami.undirected_patterns.dataStructures.HPListGraph;
import com.grami.undirected_patterns.dataStructures.Query;
import com.grami.undirected_patterns.dataStructures.myNode;
import com.grami.undirected_patterns.decomposer.Decomposer;
...
public class DFSSearch
{
	...
	//for automorphisms and non-cached com.grami.undirected_patterns.search
	...
	private void AC_3_New(Variable[] input, int freqThreshold)
	{
		...
		{			
			...
			if(oldV1Size!=v1.getListSize())
			{
				...
				//add to queue
				ArrayList<Integer> list=v1.getDistanceConstrainedWith();
				for (int j = 0; j < list.size(); j++) 
				{
			...
			if(oldV2Size!=v2.getListSize())
			{
				...
				//add to queue
				ArrayList<Integer> list=v2.getDistanceConstrainedWith();
				for (int j = 0; j < list.size(); j++) 
				{
					Variable consVar=variables[list.get(j)];
	...
	public int hasBeenPrecomputed(Variable[] autos,int[] preComputed,int index) //if returns same index should com.grami.undirected_patterns.search in it!!
	{
	...
	public void searchExistances()
	{
		...
		for (int i = variables.length-1; i >=0 ; i--) 
		{
			...
			for (Iterator<myNode> iterator = firstList.values().iterator(); iterator.hasNext();)
			{
				...
				//if already marked dont com.grami.undirected_patterns.search it
			...
			//INTO Timedout com.grami.undirected_patterns.search
