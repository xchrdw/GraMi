...
package com.grami.undirected_patterns.automorphism;
...
import com.grami.undirected_patterns.dataStructures.IntIterator;
...
//	private Variable[] currentVariables;
	...
	public Automorphism(HPListGraph<NodeType, EdgeType> graph) 
	{
		...
		for (int i = 0; i < graph.getNodeCount(); i++) 
		{
			...
			for (IntIterator currentEdges = graph.getEdgeIndices(i); currentEdges.hasNext();) 
			{
				...
				if(direction==1)
					currentNode.addreachableNode(otherNode,null);
				else if(direction ==-1)
					otherNode.addreachableNode(currentNode,null);
				else if(direction==0)
				{
					currentNode.addreachableNode(otherNode,null);
					otherNode.addreachableNode(currentNode,null);
