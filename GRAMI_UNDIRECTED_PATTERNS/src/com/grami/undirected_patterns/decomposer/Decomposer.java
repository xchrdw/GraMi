...
package com.grami.undirected_patterns.decomposer;
	...
	public void decompose()
	{
		...
		for (int i = 0; i < currentGraph.getEdgeCount(); i++) 
		{
			...
			for (int j = 0; j < newGraph.getNodeCount(); j++) 
			{
				...
				int nodeID = j; //start from here and com.grami.undirected_patterns.search !!
				if(colored[j]==1)
					continue;
				...
				while(!DFSstack.isEmpty())
				{
					...
					for (IntIterator it = newGraph.getEdgeIndices(currentNodeIDmapping); it.hasNext();) 
					{
						...
						connectedComp.addEdgeIndex(currentNodeID, otherNode, (EdgeType)"0", newGraph.getDirection(edge,currentNodeIDmapping));
